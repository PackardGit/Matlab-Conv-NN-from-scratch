...
DS1=S1;
K1 = randn(mn1,mn1,1,p_nr)*sqrt(2/mn1^2*p_nr);    %making kernel 1
...
f=f';
K2 = randn(mn2,mn2,p_nr,q_nr)*sqrt(2/mn2^2*q_nr*p_nr);    %making kernel 2
...
W=randn(nw,(im_s3/2)^2*q_nr)*sqrt(2/(im_s3/2)^2*q_nr);
b=zeros(nw,1);
V=zeros(nw,1);
y_p=zeros(nw,1);%output

%learning parameters
n=0.0050;    %learning rate
...
tic
        ...
        %zerowanie
%         DV1(1:im_s2,1:im_s2,1:p_nr)=0;
%         DC1(1:im_s2,1:im_s2,1:p_nr)=0;
%         DC1_f=DC1;
%         DS1(1:im_s2/2,1:im_s2/2,1:p_nr)=0; 
%         DK1(1:mn1,1:mn1,1,1:p_nr)=0;
%         Db1(1:im_s2,1:im_s2,p_nr)=0;
%         DH2(1:im_s3,1:im_s3,1:q_nr)=0;
%         DV2(1:im_s3,1:im_s3,1:q_nr)=0;
%         DC2(1:im_s3,1:im_s3,1:q_nr)=0;
%         DC2_f=DC2;
%         DS2(1:im_s3/2,1:im_s3/2,1:q_nr)=0;
%         Df(1:(im_s3/2)^2*q_nr)=0;
%         DK2(1:mn2,1:mn2,1:p_nr,1:q_nr)=0;
%         Db2(1:im_s3,1:im_s3,q_nr)=0;
for i = 1:nr_img 
            %% forward propagation
